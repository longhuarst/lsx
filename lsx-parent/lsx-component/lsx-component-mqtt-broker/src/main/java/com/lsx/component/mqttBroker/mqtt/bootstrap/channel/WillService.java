...
import com.lsx.component.mqttBroker.mqtt.bootstrap.bean.WillMeaasge;
...
public class WillService  implements BaseApi {
    ...
    Logger logger = LoggerFactory.getLogger(WillService.class);
    ...
    public void doSend( String deviceId) {  // 客户端断开连接后 开启遗嘱消息发送
        if(  StringUtils.isNotBlank(deviceId)&&(willMeaasges.get(deviceId))!=null){
            ...
            if(!willMeaasge.isRetain()){ // 移除
                ...
                logger.info("deviceId will message["+willMeaasge.getWillMessage()+"] is removed");
            }
        }
    }
